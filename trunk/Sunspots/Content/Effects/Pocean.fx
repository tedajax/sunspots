...
texture texSpecularMask < string name = "..\\Water\\SpecularMask.dds"; >;
...
texture texAlpha < string name = "..\\Water\\AlphaMap.dds"; >;
...
float3 waterDarkColor = float3(0.08f, 0.3f, 0.4f);
float3 waterLightColor = float3(0.21f, 0.45f, 0.6f);
...
COMMON_PS_OUTPUT CommonPS(PS_INPUT IN) {
	...
	OUT.specular = pow(saturate(reflected), 4) * tex2D(sampSpecularMask, specularMapTC);	
	//OUT.specular = tex2D(sampSpecularMask, specularMapTC);
...
float4 PS2(PS_INPUT IN) : COLOR {
	...
	lightCol = float3(.85,.85,.85);
float3 retColor = common.waterColor + common.specular * lightCol;
	//float4 retColor = float4(0,0,100,100);
